...
export function ResearcherDetailPage({
  ...
  const { data, isLoading, error, refetch } = useDetailResearcher({
    researcherId,
  });

  const handleBack = () => router.push(ROUTES.USERS.RESEARCHERS.MAIN);
  const title = data?.fullName ?? "Detalhes";

  if (isLoading) {
    return (
      <Box p={24} mx="auto">
        <Typography>Carregando pesquisador...</Typography>
      </Box>
    );
  }
